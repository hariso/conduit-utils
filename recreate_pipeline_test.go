...
func createTransformationInsertFieldPayload(connectorId string, pipelineId string, client *http.Client) (string, error) {
	fmt.Println("createTransformationStructured")

	req := CreateProcessorRequest{
		Name: "insertfieldpayload",
		Type: Processor_TYPE_TRANSFORM,
		Parent: &Processor_Parent{
			Type: Processor_Parent_TYPE_CONNECTOR,
			Id:   connectorId,
		...
		Config: &Processor_Config{
			Settings: map[string]string{
				"static.field": "greeting",
				"static.value": "marhaba",
			},
		},
	}
...
func createTransformationHTTP(client *http.Client, req CreateProcessorRequest) (string, error) {
	pipelineCreateUrl := "http://localhost:8080/v1/processors"
...
func createGeneratorSource(dataType string, client *http.Client, pipelineId string) (string, error) {
